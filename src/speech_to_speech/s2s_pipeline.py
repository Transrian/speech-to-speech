...
from speech_to_speech.arguments_classes.kyutai_tts_arguments import KyutaiTTSHandlerArguments
...
class ParsedArguments:
    ...
    kyutai_tts_handler_kwargs: KyutaiTTSHandlerArguments
...
def parse_arguments() -> ParsedArguments:
    ...
    return ParsedArguments(
        ...
        kyutai_tts_handler_kwargs=by_type[KyutaiTTSHandlerArguments],
    )
...
def prepare_all_args(
    ...
    rename_args(kyutai_tts_handler_kwargs, "kyutai_tts")
...
def _build_realtime_pipeline_unit(
    *,
    ...
    kyutai_tts_kw = deepcopy(kyutai_tts_handler_kwargs)
    ...
    handlers = _build_pipeline_handlers(
        ...
        kyutai_tts_handler_kwargs=kyutai_tts_kw,
        speculative_turns=speculative_turns,
    )
    for h in handlers:
    ...
    if module_kwargs.mode == "local":
        ...
        pool = [
            _build_realtime_pipeline_unit(
                ...
                kyutai_tts_handler_kwargs=kyutai_tts_handler_kwargs,
            )
...
def get_tts_handler(
    ...
    elif module_kwargs.tts == "kyutai-tts":
        try:
            from speech_to_speech.TTS.kyutai_tts_handler import KyutaiTTSHandler
        except ImportError as e:
            raise ImportError(
                'Kyutai TTS is optional. Install it with `pip install "speech-to-speech[kyutai]"`.'
            ) from e

        return KyutaiTTSHandler(
            stop_event,
            queue_in=lm_response_queue,
            queue_out=send_audio_chunks_queue,
            setup_args=(should_listen,),
            setup_kwargs=vars(kyutai_tts_handler_kwargs),
        )
    else:
        raise ValueError("The TTS should be either chatTTS, facebookMMS, pocket, kokoro, qwen3, or kyutai-tts")
...
def main() -> None:
    ...
    prepare_all_args(
        ...
        args.kyutai_tts_handler_kwargs,
    )
